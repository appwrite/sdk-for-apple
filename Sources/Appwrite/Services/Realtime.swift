...
open class Realtime : Service {
    ...
    private func isTornDown() -> Bool {
        let hasPendingPresence: Bool = presenceSync.sync { pendingPresence != nil }
        return activeSubscriptions.isEmpty && !hasPendingPresence
    }
    ...
    private func enqueuePendingSubscribe(subscriptionId: String) {
    ...
    private func sendPendingSubscribes() {
        ...
        let rows = Array(pendingSubscribes.values)
        pendingSubscribes.removeAll()
    ...
    public func upsertPresence(
        ...
        if socketClient == nil || !(socketClient?.isConnected ?? false) {
            Task { [weak self] in
                do {
    ...
    public func subscribe<T : Codable>(
        ...
        let subscriptionId = try generateUniqueSubscriptionId()
        ...
        return RealtimeSubscription(
            unsubscribe: { [weak self] in
                guard let self = self else { return }
                guard self.activeSubscriptions[subscriptionId] != nil else { return }
            ...
            update: { [weak self] changes in
                guard let self = self, let subscription = self.activeSubscriptions[subscriptionId] else {
                    return
                }
                ...
                self.enqueuePendingSubscribe(subscriptionId: subscriptionId)
            ...
            close: { [weak self] in
                guard let self = self else { return }
                if self.activeSubscriptions[subscriptionId] != nil {
                    self.activeSubscriptions[subscriptionId] = nil
                    self.pendingSubscribes[subscriptionId] = nil
                    self.sendUnsubscribeMessage([subscriptionId])
                }
